...
| `gira goal plan` | `advanced_orchestration` | `legacy_bullet_planning_engine` | none | `apply_mutation` | `stable_json` | creates linked child tickets from reviewed goal-plan proposals when run with --apply; --dry-run previews the same plan | docs/goal-operating-model.md, docs-site/command-reference.md |
...
| `gira ticket new` | `managed_delivery` | `none` | none | `apply_mutation` | `stable_json` | creates a GitHub issue, may set a native parent, and may optionally start it; --dry-run previews issue body, labels, and parent plan | README.md, docs-site/ticket-workflow.md, docs/dogfood.md |
