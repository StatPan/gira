...
| --- | --- | --- | --- |
| Intake / backlog layer | Project-agnostic tickets can exist before repository routing. | Partially supported. Portfolio repo issues can now act as top-level tickets and `gira portfolio plan --dry-run` can compute lowering plans. Lowering apply is specified but not implemented. | Add capability-gated `gira portfolio lower --apply` for idempotent execution issue creation and parent child-link updates. |
