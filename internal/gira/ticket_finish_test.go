...
func (r *finishRunner) Run(name string, args ...string) ([]byte, error) {
	...
	if len(queue) == 0 && key == "gh api repos/StatPan/gira/pulls/220/reviews --paginate --slurp" {
		return []byte(`[[{"state":"APPROVED","commit_id":"head220"}]]`), nil
	}
...
func TestFinishWorkJiraPrimaryBlocksDoneWhenGitHubEvidenceIncomplete(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "review blocker", prJSON: `[{"number":220,"title":"x","body":"Closes #219","state":"OPEN","url":"u","reviewDecision":"REVIEW_REQUIRED","isDraft":false,"mergeStateStatus":"BLOCKED","statusCheckRollup":[{"conclusion":"SUCCESS","status":"COMPLETED"}]}]`, blocker: "review_required_but_absent"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "review blocker" {
				if result.ReviewEvidence.Blocker != "review_required_but_absent" || result.Readiness.Review.Evidence.Blocker != "review_required_but_absent" || result.Receipt.ReviewSummary.Evidence.Blocker != "review_required_but_absent" {
					t.Fatalf("required-review blocker must be preserved in finish evidence: %+v", result)
				}
				if result.NextStep != "Request and record an approving review for the current PR head." {
					t.Fatalf("required-review remediation = %q", result.NextStep)
				}
			}
			if result.JiraTransition != nil && result.JiraTransition.Decision == "direct_transition" {
