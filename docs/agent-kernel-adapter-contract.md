...
| `gira goal status`, `gira goal dossier`, `gira goal next`, `gira goal plan --dry-run`, `gira goal finish --dry-run` | `read` or `dry_run_mutation` | `goal dossier` packages the visible goal operating state without mutation. `goal plan --dry-run` and `goal finish --dry-run` prepare child-ticket plans or receipts but do not apply. `goal next` can select work or stop. |
