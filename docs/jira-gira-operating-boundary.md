...
| --- | --- | --- | --- |
| Intake / backlog layer | Project-agnostic tickets can exist before repository routing. | Partially supported. Portfolio repo issues can now act as top-level tickets and `gira portfolio plan --dry-run` can compute lowering plans. Apply/lower behavior is future work. | Add capability-gated lowering apply after dry-run plans prove stable. |
