...
func BuildJiraParityReportWithEvidence(repo RepoRef, capability ProjectCapabilityReport, evidence JiraParityEvidence, now time.Time) JiraParityReport {
	...
	domains := []JiraParityDomain{
		buildDomain("install_detach", weights.InstallDetach, capability, evidence, []domainCheck{
			...
			{capability: "issues:write", command: "gira ops bootstrap"},
			{command: "gira ops doctor"},
			{command: "gira ops detach"},
		}),
		buildDomain("jira_migration", weights.JiraMigration, capability, evidence, []domainCheck{
			{capability: "issues:read", command: "gira ops sync --dry-run"},
			{capability: "issues:write", command: "gira ops sync --policy-mode adopt|merge|enforce"},
			{capability: "issues:write", command: "gira ops jira import"},
			{capability: "issues:read", command: "gira ops jira export"},
			{capability: "issues:write", command: "gira ops triage --apply"},
			{command: "gira ops contract crud"},
		}),
		buildDomain("daily_work_loop", weights.DailyWorkLoop, capability, evidence, []domainCheck{
			...
			{capability: "issues:write", command: "gira ticket start"},
			{capability: "pullrequests:write", command: "gira ticket pr"},
			{capability: "issues:read", command: "gira ticket status"},
			{capability: "issues:write", command: "gira ops worker claim|handoff|release"},
		}),
		buildDomain("sprint_release_report", weights.SprintRelease, capability, evidence, []domainCheck{
			...
			{capability: "issues:read", command: "gira ops report weekly"},
			{capability: "issues:read", command: "gira ops export dashboard"},
		}),
		buildDomain("governance_readiness", weights.GovernanceReadiness, capability, evidence, []domainCheck{
			{capability: "repo:settings:write", command: "gira ops guardrails sync --apply"},
			{capability: "issues:read", command: "gira ops graph validate"},
			{capability: "pullrequests:read", command: "gira ops review queue|gate"},
			{capability: "pullrequests:write", command: "gira ops merge queue --apply"},
			{command: "gira ops audit verify"},
		}),
	}
...
func DefaultJiraParityEvidence() JiraParityEvidence {
	return JiraParityEvidence{Commands: map[string]bool{
		"gira init":               true,
		"gira ops bootstrap":      true,
		"gira ops doctor":         true,
		"gira ops detach":         true,
		"gira ops sync --dry-run": true,
		"gira ops sync --policy-mode adopt|merge|enforce": true,
		"gira ops jira import":                            true,
		"gira ops jira export":                            true,
		"gira ops triage --apply":                         true,
		"gira ops contract crud":                          true,
		"gira status":                                     true,
		"gira ticket start":                               true,
		"gira ticket pr":                                  true,
		"gira ticket status":                              true,
		"gira ops worker claim|handoff|release":           true,
		"gira sprint plan|start|close":                    true,
		"gira sprint rollover":                            true,
		"gira release readiness":                          true,
		"gira ops report weekly":                          true,
		"gira ops export dashboard":                       true,
		"gira ops guardrails sync --apply":                true,
		"gira ops graph validate":                         true,
		"gira ops review queue|gate":                      true,
		"gira ops merge queue --apply":                    true,
		"gira ops audit verify":                           true,
	}}
}
